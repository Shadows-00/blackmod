    ...
    client.on("message", async message => {
      ...
      if (nextLevel <= currentlyData.XP) {
        ...
        message.channel.send(
          `${message.member}, **HELAL LAN SANA!** Seviye atladın ve **${currentlyData.Level}. seviyeye** ulaştın. 🎉`
        );
      }
...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    let embed = new MessageEmbed().setThumbnail(user.user.avatarURL({dynamic: true, size: 2048})).setAuthor(user.displayName, user.user.avatarURL({dynamic: true, size: 2048})).setFooter("BLACK").setColor(client.randomColor()).setTimestamp();
