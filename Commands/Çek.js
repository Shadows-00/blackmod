...
module.exports.execute = async(client, message, args, ayar, emoji) => {
  ...
  if (!uye) return message.channel.send(embed.setDescription("Kimi Çekecem Amk Onu Söylemiyorsun!")).then(x => x.delete({timeout: 5000}));
  if (!message.member.voice.channel || !uye.voice.channel || message.member.voice.channelID == uye.voice.channelID) return message.channel.send(embed.setDescription("ADAM SESDE DEĞİL KODUMUN SALAĞI!")).then(x => x.delete({timeout: 5000}));
